<div align="center">

[![Typing SVG](https://readme-typing-svg.demolab.com?font=Fira+Code&weight=600&size=28&pause=1000&color=58A6FF&center=true&vCenter=true&width=600&lines=Hey%2C+I'm+Arohan+%F0%9F%91%8B;Builder.+Breaker.+Fixer.;Systems+%7C+Hardware+%7C+Real-world+code)](https://git.io/typing-svg)

</div>

<p align="center">
    <img src="https://skillicons.dev/icons?i=py,c,cpp,git,raspberrypi,arduino,selenium,django,autocad,blender,figma,css" />
</p>

---

### About Me

Electronics and computers student based in **Delhi, India** — building at the intersection of hardware, systems, and real-world impact.

I started with fun builds and gradually shifted toward real constraints: hardware limits, unreliable data, scale, and problems that actually matter. Right now that means competing in the **European Rover Challenge 2026** with a Mars rover team, and developing **TerraScansi** — satellite data tooling that helps governments and NGOs tackle urban heat, pollution, and resource waste. *Delhi, we see you.*

Outside of that: I design for events and visual identity, make music seriously, and have a secret audio automation project you'll find out about soon enough.

Always up to learn, collaborate, and ship something real.

---

### 🚧 Active Projects

| Project | Description | Stack | Status |
|---|---|---|---|
| **[Mars Rover](https://roverchallenge.eu/team/team-vyomagami-2026/)** | Autonomous rover for European Rover Challenge 2026 | C++, ROS, Python | 🟢 Active |
| **[TerraScansi](https://github.com/arungunayo/Terrascansi)** | Satellite data platform for urban resource optimization | Python, Streamlit, Geospatial APIs | 🟢 Active |
| **ImGui React** | React-style reactive UI framework for C++ / Dear ImGui | C++, Dear ImGui | 🔨 WIP |
| **Quadcopter Drones x2** | Drone systems, avionics, and flight controller design | Hardware, C | 🔨 WIP (1 done) |
| **Secret Audio Project** | Shh. | 🔒 | Locked |

---

### ✅ Shipped

| Project | Description |
|---|---|
| **Zelda / Star Wars Inspired Games** | Original games built from scratch in Pygame |
| **Irrigation Rover** | Real-time soil moisture-driven field rover (school robotics) |
| **Price Comparison Scraper** | Amazon × Flipkart real-time scraper with Selenium + BeautifulSoup |
| **ISBN Book Finder** | Book lookup with metadata retrieval |
| **Event Promotion Work** | Social media design & visual campaigns |

---

### Technologies & Tools

**Core**

![Python](https://img.shields.io/badge/Python-3776AB?style=flat-square&logo=python&logoColor=white)
![C](https://img.shields.io/badge/C-00599C?style=flat-square&logo=c&logoColor=white)
![C++](https://img.shields.io/badge/C++-004482?style=flat-square&logo=c%2B%2B&logoColor=white)

**Hardware & Embedded**

![Raspberry Pi](https://img.shields.io/badge/Raspberry%20Pi-A22846?style=flat-square&logo=raspberrypi&logoColor=white)
![Arduino](https://img.shields.io/badge/Arduino-00979D?style=flat-square&logo=arduino&logoColor=white)

**Data, Geo & APIs**

![NumPy](https://img.shields.io/badge/NumPy-013243?style=flat-square&logo=numpy&logoColor=white)
![Matplotlib](https://img.shields.io/badge/Matplotlib-11557c?style=flat-square&logo=python&logoColor=white)
![Streamlit](https://img.shields.io/badge/Streamlit-FF4B4B?style=flat-square&logo=streamlit&logoColor=white)
![Google APIs](https://img.shields.io/badge/Google%20APIs-4285F4?style=flat-square&logo=google&logoColor=white)
![Satellite Data](https://img.shields.io/badge/Satellite%20Imagery-1a73e8?style=flat-square&logo=googlemaps&logoColor=white)

**GUI & Scraping**

![Dear ImGui](https://img.shields.io/badge/Dear%20ImGui-555555?style=flat-square&logo=cplusplus&logoColor=white)
![Pygame](https://img.shields.io/badge/Pygame-008000?style=flat-square&logo=python&logoColor=white)
![Selenium](https://img.shields.io/badge/Selenium-43B02A?style=flat-square&logo=selenium&logoColor=white)
![Beautiful Soup](https://img.shields.io/badge/BeautifulSoup-4B8BBE?style=flat-square&logo=python&logoColor=white)

**Design**

![Figma](https://img.shields.io/badge/Figma-F24E1E?style=flat-square&logo=figma&logoColor=white)
![Adobe](https://img.shields.io/badge/Adobe%20Suite-FF0000?style=flat-square&logo=adobe&logoColor=white)
![Blender](https://img.shields.io/badge/Blender-F5792A?style=flat-square&logo=blender&logoColor=white)
![AutoCAD](https://img.shields.io/badge/AutoCAD-E51050?style=flat-square&logo=autodesk&logoColor=white)

---

### 📊 GitHub Stats

<div align="center">

<img src="https://github-readme-stats.vercel.app/api?username=arungunayo&show_icons=true&theme=tokyonight&hide_border=true&count_private=true" height="160" />
<img src="https://github-readme-stats.vercel.app/api/top-langs/?username=arungunayo&layout=compact&theme=tokyonight&hide_border=true&langs_count=6" height="160" />

![GitHub Streak](https://streak-stats.demolab.com?user=arungunayo&theme=tokyonight&hide_border=true)

</div>

---

### 📫 Connect

[![Email](https://img.shields.io/badge/Email-EA4335?style=flat-square&logo=gmail&logoColor=white)](mailto:projectsarungunayo@gmail.com)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-0A66C2?style=flat-square&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/arohan-arora/)

![Profile Views](https://komarev.com/ghpvc/?username=arungunayo&style=flat-square&color=58A6FF)
